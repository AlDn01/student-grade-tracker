...
def get_grades_from_user():
    """
    Ask the user to enter grades and return them as a list of floats.

    The user should enter grades separated by commas.
    Example: 90, 85, 77

    Returns:
        list[float]: A validated list of grades between 0 and 100.
    """
    while True:
...
def add_student_flow(tracker):
    """
    Handle the full flow for adding a new student.

    Args:
        tracker (GradeTracker): The active GradeTracker instance.
    """
    print("\n--- Add New Student ---")
...
def update_grades_flow(tracker):
    """
    Handle the full flow for updating an existing student's grades.
...
def delete_student_flow(tracker):
    """
    Handle the full flow for deleting a student record.
...
def main():
    ...
    Loads saved data from CSV on startup, presents the menu in a loop,
    and saves data when the user chooses to exit.
    """
